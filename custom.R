...
#Generation date: Tue Nov 16 16:36:10 2021
#
...
transform <- function(data, model) {
  data[is.na(data)]<-0
  data$funded_amnt<-ifelse(data$funded_amnt>0,log(data$funded_amnt),0)
...
score <- function(data, model, ...) {
   ...
   pred<-data.frame(predict(model,newdata=data,type="prob"))
   colnames(pred)<-c("0","1")
   return(pred)
}
#
